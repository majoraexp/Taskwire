...
class Card(QFrame):
    ...
    """
    def __init__(self, title="", header_spacing=10):
        """
        Initializes the Card widget.

        Args:
            title (str): The title to display on the card. Defaults to an empty string.
            header_spacing (int): Spacing between title and content. Defaults to 10.
        """
        super().__init__()
        ...
        if title:
            ...
            self.layout.addSpacing(header_spacing)
...
class CircularGauge(QWidget):
    ...
    def __init__(self):
        ...
        self.used_gb = 0
        self.total_gb = 0
        self.setMinimumSize(200, 200) # Reverted to Linux size
    ...
    def mouseMoveEvent(self, event): # pylint: disable=C0103
        ...
        size = min(rect.width(), rect.height()) - 20 # Increased padding for text
        radius = size / 2
        center = rect.center()
    ...
    def paintEvent(self, event): # pylint: disable=C0103,W0613
        ...
        draw_internal_text(f"{self.percent:.1f}%", -5, 26, bold=True)
        # Used GiB (Smaller text below percentage)
        draw_internal_text(f"{self.used_gb:.1f} GiB", 18, 14, bold=True)
...
class TempGraphWidget(Card):
    ...
    def __init__(self):
        ...
        self.history = {}
        self.sensor_states = {} # name -> bool (checked or not)
        ...
        # Legend / Values area (Scrollable)
        self.scroll_area = QScrollArea()
        self.scroll_area.setWidgetResizable(True)
        self.scroll_area.setFixedHeight(100) # Limit height to prevent expansion
        ...
        self.legend_layout = QVBoxLayout(self.scroll_content)
    ...
    def update_data(self, temp_data):
        ...
        for i, (name, value) in enumerate(temp_data.items()):
            # Determine Default State if new
            if name not in self.sensor_states:
                # Default Logic:
                # 1. Hide if value is 0
                # 2. Hide if name is generic "Temperature #", "Thermal Zone #"
                is_generic = bool(re.search(r'(Temperature|Thermal Zone)\s*#?\d+', name, re.IGNORECASE))
                is_zero = (value == 0)
                
                self.sensor_states[name] = not (is_generic or is_zero)

            color = self.colors[i % len(self.colors)]
            color_hex = color.name()
            
            # Combine name and value into a single CheckBox text (HTML not supported in CheckBox text directly in same way)
            # We use QCheckBox and set a stylesheet or use a layout. 
            # Simplest: QCheckBox text is name, Label for value? 
            # Or just QCheckBox with rich text (PyQt CheckBoxes support HTML mostly).
            
            name_escaped = html.escape(str(name))
            display_text = (  
                f"<span style='color: {color_hex}; font-weight: bold;'>{name_escaped}:</span> "
                f"<span style='color: {ModernTheme.TEXT_PRIMARY};'>{value:.1f}°C</span>"
            )
            
            if name not in self.legend_items:
                # Create Row Widget/Layout
                row_widget = QWidget()
                ...
                chk = QCheckBox()
                chk.setChecked(self.sensor_states[name])
                chk.setStyleSheet(f"QCheckBox {{ color: {ModernTheme.TEXT_PRIMARY}; }}")
                chk.stateChanged.connect(lambda state, n=name: self.on_sensor_toggled(n, state))
                
                lbl = QLabel(display_text)
                lbl.setStyleSheet(f"color: {ModernTheme.TEXT_PRIMARY}; font-size: 11px;")
                ...
                self.legend_items[name] = (chk, lbl)
            else:
                chk, lbl = self.legend_items[name]
                lbl.setText(display_text)
                # Ensure check state sync
                if chk.isChecked() != self.sensor_states[name]:
    ...
    def on_sensor_toggled(self, name, state):
        self.sensor_states[name] = (state == 2) # 2 is Checked
        self.graph_area.update()
    ...
    def update_tooltip(self):
        ...
        if self.tooltip_widget.isVisible():
            ...
            # Map screen index to data index (Right Aligned)
            screen_idx = index
    ...
    def paint_graph(self, event): # pylint: disable=C0103,W0613
        ...
        for i, (name, points_deque) in enumerate(self.history.items()):
            ...
            for j, val in enumerate(points):
                # Treat None as min_temp (bottom)
                draw_val = min_temp if val is None else val

                x = start_x + j * step_x
                norm = (draw_val - min_temp) / temp_range
...
class CpuHistoryWidget(Card):
    ...
    def update_data(self, cpu_percent):
        ...
        if self.tooltip_widget.isVisible() and self.hover_index != -1:
            ...
            screen_idx = self.hover_index
            offset = (self.maxlen - 1) - screen_idx
            data_idx = (len(self.data_points) - 1) - offset
            ...
            if 0 <= data_idx < len(self.data_points):
                val = self.data_points[data_idx]
                if val is None:
    ...
    def paint_graph(self, event): # pylint: disable=C0103,W0613
        ...
        # Calculate start X (Oldest point) so Newest point is at Width
        start_x = width - (num_points - 1) * step_x
        
        path = QPainterPath()
        path.moveTo(start_x, graph_h) # Start bottom-left relative to data
        
        for i, val in enumerate(points):
            # Treat None as 0 for drawing
            draw_val = 0.0 if val is None else val
            
            x = start_x + i * step_x
            y = graph_h - (draw_val / 100 * graph_h)
            path.lineTo(x, y)
            
        path.lineTo(width, graph_h) # Drop down at right edge
        path.lineTo(start_x, graph_h) # Return to start
        path.closeSubpath()
        ...
        if self.hover_index != -1:
            ...
            if 0 <= data_idx < len(points):
                val = points[data_idx]
                # Treat None as 0 for dot position
                draw_val = 0.0 if val is None else val
                
                hx = screen_idx * step_x
                hy = graph_h - (draw_val / 100 * graph_h)
        ...
        text = "NA" if current_val is None else f"{current_val:.1f}%"
        ...
        text_rect = QRectF(width - 200, graph_h - 60, 190, 60)
    ...
    def update_data(self, overall, per_core, freqs=None):
        ...
        if not self.bars:
            for i, _ in enumerate(per_core):
                ...
                bar.setFixedHeight(3) # Smaller height
...
class MemoryWidget(Card):
    ...
    def __init__(self):
        ...
        self.used_label_top.setStyleSheet(f"color: {ModernTheme.TEXT_PRIMARY}; font-size: 12px;") # Brighter white
        ...
        self.total_label_bottom.setStyleSheet(f"color: {ModernTheme.TEXT_PRIMARY}; font-size: 12px;") # Brighter white
        self.layout.addWidget(self.total_label_bottom)
    
    def refresh_theme(self):
        """Refreshes the widget's colors based on the current ModernTheme."""
        self.used_label_top.setStyleSheet(f"color: {ModernTheme.TEXT_PRIMARY}; font-size: 12px;")
        self.total_label_bottom.setStyleSheet(f"color: {ModernTheme.TEXT_PRIMARY}; font-size: 12px;")
        self.gauge.update()
...
class ModernDriveIcon(QAbstractButton):
    ...
    def __init__(self, parent=None):
        ...
        """
        super().__init__(parent)
        self.setFixedSize(55, 55) # Increased size
    ...
    def paintEvent(self, event): # pylint: disable=C0103,W0613
        ...
        # Scale painter to match 45->55 size increase (Base drawing logic assumes 45x45)
        # Original logic was based on 45x45 widget.
        # w, h are now 55, 55. 
        # Scale factor = 55/45 ~= 1.22
        painter.scale(self.width() / 45.0, self.height() / 45.0)

        w = 45 # Virtual width for drawing logic
        h = 45 # Virtual width for drawing logic
...
class DiskWidget(Card):
    ...
    def __init__(self):
        ...
        """
        super().__init__("Disk Usage", header_spacing=2)
        # Override title style for larger font (18px)
        self.title_label.setStyleSheet(f"font-size: 18px; font-weight: bold; color: {ModernTheme.TEXT_SECONDARY};")
        ...
        # Spacer to push icons down (centering)
        self.layout.addStretch(1)
        
        # Icons Layout
        # Icons Layout (Scrollable)
        self.scroll_area = QScrollArea()
        self.scroll_area.setWidgetResizable(True)
        self.scroll_area.setFixedHeight(90) # Height for larger icons + scrollbar
        self.scroll_area.setFrameShape(QFrame.Shape.NoFrame)
        self.scroll_area.setStyleSheet("background: transparent; border: none;")
        ...
        self.icons_widget.setStyleSheet("background: transparent;")
        ...
        # Spacer to push icons up (centering)
        self.layout.addStretch(1)

        # Selected Drive Name
        self.model_label = QLabel("Scanning...")
        self.model_label.setStyleSheet(
            f"color: {ModernTheme.ACCENT_CYAN}; font-weight: bold; font-size: 18px;"
        )
...
class DiskIOWidget(Card):
    def __init__(self):
        ...
        self.update_interval = 0
        self.last_update_time = 0
        self.read_history = deque([None]*self.maxlen, maxlen=self.maxlen)
        self.write_history = deque([None]*self.maxlen, maxlen=self.maxlen)
        ...
        self.read_bar.setFixedHeight(4)
        ...
        self.write_bar.setFixedHeight(4)
    ...
    def update_data(self, stats):
        ...
        if self.tooltip_widget.isVisible() and self.hover_index != -1:
            ...
            screen_idx = self.hover_index
            offset = (self.maxlen - 1) - screen_idx
            data_idx = (len(self.read_history) - 1) - offset
            ...
            if 0 <= data_idx < len(self.read_history):
                r_val = self.read_history[data_idx]
                w_val = self.write_history[data_idx]
    ...
    def paint_graph(self, event):
        ...
        max_r = max((v for v in self.read_history if v is not None), default=0)
        max_w = max((v for v in self.write_history if v is not None), default=0)
        max_val = max(max_r, max_w)
        ...
        def draw_line(data_deque, color_hex):
            ...
            # Draw Dot if hovered
            # Correct index mapping needed for hover logic too, but visual dot logic here:
            if self.hover_index != -1:
                # Map hover index (from 0..maxlen-1 left-to-right on screen) 
                # to data index.
                # Screen X -> Index 0..MaxLen
                # Data is right aligned.
                # Screen Index 0 = Oldest possible time (empty if not full)
                # Screen Index Max = Newest possible time (data[-1])
                
                # Logic:
                # screen_idx = self.hover_index
                # offset from right = (maxlen - 1) - screen_idx
                # data_idx = (len(points) - 1) - offset
...
class NetworkWidget(Card):
    def __init__(self):
        ...
        # Style
        # Using stylesheet for strict sizing
        self.up_label.setStyleSheet(f"color: {ModernTheme.ACCENT_CYAN}; font-size: 18px; font-weight: bold;")
        self.down_label.setStyleSheet(f"color: {ModernTheme.ACCENT_GREEN}; font-size: 18px; font-weight: bold;")
...
class ModernGaugeWidget(Card):
    ...
    def __init__(self, title, color_hex=ModernTheme.ACCENT_CYAN):
        ...
        self.gauge_area.setSizePolicy(
            self.gauge_area.sizePolicy().horizontalPolicy(),
            self.gauge_area.sizePolicy().verticalPolicy()
        )
        self.gauge_area.paintEvent = self.paint_gauge
        self.layout.addWidget(self.gauge_area, 1) # Add stretch factor 1 to expand
    ...
    def paint_gauge(self, event): # pylint: disable=C0103,W0613
        ...
        # Use almost full space, just small padding
        size = min(w, h) - 10 
        x = (w - size) / 2
        y = (h - size) / 2
...
class FanGraphWidget(Card):
    ...
    def __init__(self):
        ...
        self.legend_layout = QVBoxLayout(self.scroll_content)
        self.legend_layout.setContentsMargins(0, 5, 0, 0)
        self.legend_layout.setAlignment(Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft)
    ...
    def update_data(self, fan_data):
        ...
        for i, (name, value) in enumerate(fan_data.items()):
            ...
            name_escaped = html.escape(str(name))
            display_text = f"<span style='color: {color_hex}; font-weight: bold;'>|</span> {name_escaped}: <span style='color: {ModernTheme.TEXT_PRIMARY}; font-weight: bold;'>{int(value)} RPM</span>"
            ...
            if name not in self.legend_items:
                ...
                chk = QCheckBox() # No text on check
                chk.setChecked(self.sensor_states[name])
                chk.setStyleSheet(f"QCheckBox {{ color: {ModernTheme.TEXT_PRIMARY}; }}")
                chk.stateChanged.connect(lambda state, n=name: self.on_sensor_toggled(n, state))
    ...
    def on_sensor_toggled(self, name, state):
        self.sensor_states[name] = (state == 2)
        self.graph_area.update()
