...
echo ------------------------------------------------
echo Taskwire Windows Nuitka Build (OneDir / Folder Mode)
echo ------------------------------------------------
echo This mode creates a folder instead of a single EXE.
echo This usually avoids false-positive antivirus detections.
echo.
...
:: 6. Run Nuitka (OneDir Mode)
echo [6/6] Compiling with Nuitka (Standalone Folder)...
:: Removed --onefile
:: Added --standalone (implied by onefile but explicit here)
...
echo.
echo [Build Success]
echo Output Directory: Windows_port_Nuitka\output_onedir\Taskwire.dist\
echo Executable: Taskwire.exe
echo ------------------------------------------------
pause
